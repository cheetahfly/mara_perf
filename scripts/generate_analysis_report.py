...
for case in consecutive_big:
    if case['date1'] and case['date2']:
        try:
            d1 = pd.to_datetime(case['date1'])
            d2 = pd.to_datetime(case['date2'])
            if pd.notna(d1) and pd.notna(d2):
                days = (d2 - d1).days
                if days <= 730 and days > 0:
                    case['days'] = days
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="flow">
                ...
                <span class="flow-arrow">→</span>
                <div class="flow-step">30+岁男性全马<3小时</div>
            </div>
            <table>
                <tr>
                    <th>筛选条件</th>
        ...
        <div class="card">
            ...
            <h3>2. 改进幅度分布</h3>
            <p>计算所有运动员相邻两次比赛的成绩改进幅度（秒）</p>
            <div class="formula">
                改进幅度 = 前一次成绩 - 后一次成绩（正数表示进步）
            </div>
        ...
        <div class="card">
            ...
            <div class="formula">
                ...
                = {np.sum(improvements >= 223)/len(improvements)*100:.4f}% × {np.sum(improvements >= 179)/len(improvements)*100:.4f}%
            ...
            <table>
                ...
                <tr>
                    <td>单次改进 ≥ 402秒</td>
                    <td>{np.sum(improvements >= 402)} / {len(improvements)}</td>
                    <td><span class="badge badge-danger">{np.sum(improvements >= 402)/len(improvements)*100:.4f}%</span></td>
                </tr>
                <tr>
                    <td>连续两次改进各≥180秒</td>
        ...
        <div class="card">
            ...
            <div class="success-box">
                <p style="font-size: 1.1rem; margin-bottom: 1rem;">在 {len(improvements):,} 对相邻成绩中，仅找到 <strong>1例</strong> 符合条件的连续大进步案例：</p>
            </div>
"""
    ...
    html += f"""
            <table>
                ...
                <tr>
                    ...
                    <td>{int(case['time1'])}秒 → {int(case['time2'])}秒<br>(改进 {case['imp1']:.0f}秒)</td>
                    <td>{int(case['time2'])}秒 → {int(case['time3'])}秒<br>(改进 {case['imp2']:.0f}秒)</td>
...
html += f"""
        ...
        <div class="card">
            ...
            <table>
                ...
                <tr>
                    <td>数据文件</td>
                    <td><code>data/marathon_tidy.db</code> - 统一去重的SQLite数据库</td>
                </tr>
                <tr>
                    <td>原始数据</td>
                    <td>
                        <ul style="margin: 0.5rem 0 0 1.5rem;">
                            <li><strong>World Athletics</strong>: 来自 <code>world-athletics-database</code> GitHub仓库</li>
                            <li><strong>7城市马拉松</strong>: Berlin, Boston, Chicago, Honolulu, London, New York City, Portland</li>
                            <li>来自 <code>marathon-results</code> GitHub仓库</li>
                        </ul>
                    </td>
                </tr>
                <tr>
                    <td>数据量</td>
                    <td>总运动员 {len(df_30plus)} 人，总成绩记录 {len(df_30plus)} 条</td>
                </tr>
                <tr>
                    <td>分析条件</td>
                    <td>全马 | 男性 | 30+岁 | 跑进3小时</td>
                </tr>
                <tr>
                    <td>有效样本</td>
                    <td>{len(improvements):,} 对相邻成绩用于统计分析</td>
                </tr>
            </table>
        </div>
    ...
    <script>
        // 改进幅度分布直方图
        const improvementData = """ + json.dumps(improvements.tolist()) + """;

        // 创建直方图数据
        const bins = [];
        const counts = [];
        const step = 60; // 60秒一个区间
        for (let i = -300; i <= 600; i += step) {{
            bins.push(i);
            counts.push(0);
        }}

        improvementData.forEach(v => {{
            const idx = Math.floor((v + 300) / step);
            if (idx >= 0 && idx < counts.length) counts[idx]++;
        }});

        const labels = bins.map((b, i) => {{
            if (b < 0) return (b-step) + 's';
            if (i === 0) return '< -300s';
            return b + 's';
        }});
...
print(f"分析报告已生成: data/marathon_anomaly_report.html")
